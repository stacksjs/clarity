...
describe('Logger', () => {
  ...
  describe('Basic Logging', () => {
    ...
    //   expect(logger.getLevel()).toBe('debug') // Check configured level is 'debug'
    ...
    it('should format messages with timestamps', async () => {
      ...
      // Create a new logger with encryption disabled for this test
      logger = new Logger('test', {
        logDirectory: TEST_LOG_DIR,
        level: 'debug',
        timestamp: fakeTime,
        rotation: {
          ...
          frequency: 'daily',
          encrypt: false, // Disable encryption for this test
        },
      })
    ...
    //       if (textLogger.getFormat()) {
    //         expect(textLogger.getFormat()).toBe('text')
    //       }
    ...
    //         const maxFiles = logger.getRotationConfig() && typeof logger.getRotationConfig() !== 'boolean'
    //           ? (logger.getRotationConfig() as RotationConfig).maxFiles || 3
    //           : 3
    ...
    //     console.error(`Current isBrowser value: ${logger.isBrowserMode()}`)
    ...
    //     expect(logger.isServerMode()).toBe(!logger.isBrowserMode())
    ...
    //     expect(tempLogger.isBrowserMode()).toBe(logger.isBrowserMode())
    ...
    //     console.error(`Logger environment: isBrowser=${logger.isBrowserMode()}, isServer=${logger.isServerMode()}`)
    ...
    //     console.error(`Environment behavior test: isBrowser=${logger.isBrowserMode()}, isServer=${logger.isServerMode()}`)
    ...
    //     expect(defaultLogger.getLogDirectory()).toBeDefined()
    ...
    //     expect(customLogger.getLevel()).toBe('warning')
    //     expect(customLogger.getLogDirectory()).toBe(join(TEST_LOG_DIR, 'custom'))

    //     // Check rotation config - these might be nested in config.rotation
    //     expect(customLogger.getRotationConfig()).toBeDefined()
    ...
    //     expect(invalidLevelLogger.getLevel()).toBeDefined()
    ...
    //     expect(loggerWithUndefinedProps.getConfig()).toBeDefined()
    //     // The config should have a logDirectory property that's defined
    //     expect(loggerWithUndefinedProps.getLogDirectory()).toBeDefined()
